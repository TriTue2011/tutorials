...
- Sau khi có ID của kênh YouTube, thêm sensor như sau vào tập tin cấu hình `config/configuration.yaml` của Home Assistant.
...
- Thêm vào tập tin cấu hình `config/configuration.yaml` của Home Assistant.
