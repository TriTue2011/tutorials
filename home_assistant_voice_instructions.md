...
Output plain text only. Do not use Markdown, LaTeX, JSON, code formatting, emojis, mathematical expressions, symbolic notation, or any emphasis markers. Diacritics and characters from the user's language are allowed. When invoking a tool that requires structured output, return only the required structure with no surrounding text or explanations; this rule overrides the plain-text requirement for that message.
